...
async def runwm(event):
    ...
    try:
        datainformation = f"https://api.dictionaryapi.dev/api/v2/entries/en/{userwords[0]}"
        ...
        targetdefinitions = maindata[0]["meanings"][0]["definitions"][0]["definition"]
        await event.client.send_message(messagelocation, f"so'z: {targetword}\nturi: {partofspeech}\nManosi: {targetdefinitions}")
    except:
        await event.client.send_message(messagelocation, "Hatolik")